...
int main(int argc, char *argv[])
{
	...
	if ((socksd = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0)
       printf("socketsd() failed.\n");
    struct timeval timeout;
    timeout.tv_sec = 1;
    timeout.tv_usec = 0;

    setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout));
	setsockopt(socksd, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout));
    int x = 1;
	...
	if(setsockopt(socksd,SOL_SOCKET,SO_BINDTODEVICE,(char *)&if_eth1,sizeof(if_eth1))<0){
		printf("bind socket to eth1 error\n");
	}
...
void recvMSG(int type, int subType)
{
    switch(type){
        ...
        case 8:{ // Read the DHCP ACK, send nothing
            svrAddrLen = sizeof(svrAddr);
            if ((recvSize = recvfrom(socksd, recvBuf, MAX_SIZE, 0,(struct sockaddr *) &svrAddr, &svrAddrLen)) < 312)
                printf("recvfrom() failed.\n");
...
void sendMSG(int type, int subType)
{
    ...
    if(type == 1 || type == 3 || type == 7 || type == 8)
    {
        if(type == 7 && getIP() == 0x01010101)
			...
			length = sendto(socksd, sendBuf, sendSize, 0, (struct sockaddr *) &sendAddr, sizeof(sendAddr));
		}
    }

...
#define MAX_SIZE 1000 /* Longest string to echo */
int sock; /* Socket */
int socksd;
...
if ((socksd = socket(PF_INET, SOCK_DGRAM, 0)) < 0)
    ...
    while(1)
    {
		...
		if ((recvMsgSize = recvfrom(socksd, recvBuf, MAX_SIZE, 0,(struct sockaddr *) &cltAddr, &cliAddrLen)) < 0)
            printf("recvfrom() failed.\n");
        if (recvMsgSize < 312)
            continue;
        ...
        if((recvBuf[242] == 0x01 || recvBuf[242] == 0x03 || recvBuf[242] == 0x08) && (sendSize >= 312))
        {
            int length = 0;
            if(broadcastFlag == 0)
                length = sendto(socksd, sendBuf, sendSize, 0, (struct sockaddr *) &cltAddr, sizeof(cltAddr));
